...
def worker_fn(star_id, constants):
    ...
    # no MAP file, plot the LS fit output
    else:
        fig, ax = plt.subplots(ncols=1, nrows=n_cbvs+3, figsize=(7, 20), sharex=True, sharey=True)

        # row 0 is the raw fluxes, duplicate them to guide the eye
        title = f'Mag: {t_mag} Var: {var:.3f}'
        fig.suptitle(title)
        ax[0].plot(flux, 'k.', label='raw data')
        ax[0].legend()
        ...
        # plot the commbined cond CBVs
        ax[n_cbvs+1].plot(cond_cbvs, '.', color='orange', label='LS')
        ax[n_cbvs+1].legend()

        # then the detrended lc cond
        ax[n_cbvs+2].plot(corrected_cond, 'g.', label='Cotrended LS')
        ax[n_cbvs+2].legend()
